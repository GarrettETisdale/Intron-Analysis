...
with open('Chlamydomonas_reinhardtii_Extracted_Intron_Data.csv') as csvfile:
...
with open('Chlamydomonas_reinhardtii_Extracted_Expression_Data.csv') as csvfile:
    readCSV = csv.reader(csvfile, delimiter=',')
    next(readCSV)
    for row in readCSV:
        gene_number_temp_2.append(row[0])
        expression_temp_2.append(float(row[1]))
...
plt.title('Chlamydomonas reinhardtii - Total Intron Length vs Gene Expression', fontsize=size_font)
